...
extern "C"
{
    ...
    void krnl(float *in1, float *in2, float *out_r,const unsigned int n,const unsigned int q)
    {
...
#pragma HLS LOOP_TRIPCOUNT min = c_m *c_q max = c_m * c_q
    ...
    // and write results into C
        nopart1:
            ...
            {
#pragma HLS LOOP_TRIPCOUNT min = 1 max = c_n
            nopart2:
...
#pragma HLS LOOP_TRIPCOUNT min = c_m max = c_m
                nopart3:
                    for (int j = 0; j < c_q; j++)
                    {
#pragma HLS LOOP_TRIPCOUNT min = 1 max = c_q
...
//     writeC:
//         for (int itr = 0, i = 0, j = 0; itr < c_n*c_q; itr++, j++)
//         {
// #pragma HLS LOOP_TRIPCOUNT min = c_n *c_q max = c_n * c_q
//             if (j == c_q)
//             {
//                 j = 0;
//                 i++;
...
//     }
    ...
    write:
        for(unsigned int i=0;i<q;i++){
            #pragma HLS LOOP TRIPCOUNT min=1 max=c_q
            out_r[i*2]=max_val[i];
            out_r[i*2 + 1] = record_no[i];
        }
    }
    // for(unsigned int i=0;i<c_q;i++)
    //     printf("%d:%f\n", record_no[i], max_val[i]);
    // }
}